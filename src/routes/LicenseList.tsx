...
export function LicenseList({ showBackButton }: { showBackButton: boolean }) {
  ...
  const [licenseTextParameters, setLicenseTextParameters] = useState<
    Parameters<typeof licenseRequests.text> | undefined
  >(undefined);
  ...
  function createLicenseList(
    component: Parameters<typeof licenseRequests.text>[0],
    index: LicenseIndexDto | undefined,
  ) {
    return (
        ...
        {index ?
          <ul className={styles.licenseList}>
            {Object.entries(index).map(([packageName, licenseHash]) => (
              <li key={packageName}>
                <Link
                  href={licenseRequests.textUrl(component, licenseHash).toString()}
                  target="_blank"
                  onClick={(e) => {
                    e.preventDefault();
  ...
  return (
    <div className={globalStyles.container}>
      <Dialog
        modalType="modal"
        open={licenseTextVisible}
        onOpenChange={(_, d) => (d.open || licenseText !== undefined) && setLicenseTextVisible(d.open)}>
            ...
            <DialogActions>
              {packageName && (
                <RouterButton
                  ...
                  href={"https://www.npmjs.com/package/" + packageName.slice(0, packageName?.indexOf("@", 1))}
                  target="_blank">
                  Visualizza su npmjs.com
                </RouterButton>
              )}
              <Button
                appearance="primary"
                onClick={() => setLicenseTextVisible(false)}
                disabled={licenseText === undefined}>
                Chiudi
              </Button>
            </DialogActions>
          </DialogBody>
      ...
      <Card className={globalStyles.card}>
        <CardHeader
          ...
          action={
            showBackButton ?
              <Button appearance="subtle" icon={<DismissRegular />} onClick={() => window.history.back()} />
            : undefined
          }
        />
      </Card>
