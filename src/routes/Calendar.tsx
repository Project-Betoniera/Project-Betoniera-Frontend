...
} from "../components/calendar/CalendarSelector";
...
export function Calendar() {
  ...
  const calendarView =
    isCurrentViewMonth ? generateMonth(dateTime).flat()
    : screenMediaQuery ? generateShortWeek(dateTime)
    : generateWeek(dateTime);
  ...
  async function getEvents(
    calendar: Calendar,
    from: Date = calendarView[0],
    to: Date = calendarView[calendarView.length - 1],
  ) {
    setLoading(true);
    ...
    return newEvents
      .filter(
        (event) =>
          events.find((item) => item.id === event.id && item.selectionId === calendar.selection.id) === undefined,
      )
      .map((event) => {
  ...
  function updateCalendarTitle() {
    if (calendars.length == 1) {
      ...
      setCalendarTitle(
        `Calendario per ${calendar.selection.type === "classroom" ? calendar.selection.fullName : calendar.selection.shortName}`,
      );
  ...
  function addCalendars(...calendars: Calendar[]) {
    setCalendars((oldValue) => {
      ...
      if (newValue.length === oldValue.length) return oldValue;
  ...
  function renderCurrentCalendarView() {
    function renderSkeletons() {
      ...
      return <Skeleton className={styles.eventContainer}>{elements}</Skeleton>;
    }
    ...
    function renderPreviewEvents(events: ExtendedEventDto[]) {
      ...
      return events.map((event, index) => (
        <Card key={index} className={styles.event} style={{ backgroundColor: getColorValue(event.color) }}>
                      ...
                      filteredEvents.length > 0 ?
                        filteredEvents.map((event, index) => (
                          <EventDetails
                            ...
                            event={event}
                            title="subject"
                            backgroundColor={getColorValue(calendar.color)}
                          />
                        ))
          ...
          <DialogTrigger>
            <Card
              ...
              className={mergeClasses(
                styles.card,
                now.toLocaleDateString() === day.toLocaleDateString() && styles.todayBadge,
              )}>
              <div className={styles.eventHeader}>
                <Subtitle2>{day.toLocaleDateString([], { day: "numeric" })}</Subtitle2>
                {isCurrentViewMonth && filteredEvents.length > 0 ?
                  <div>{renderBadges(day, filteredCalendars, filteredEvents)}</div>
                : undefined}
              </div>
              <div
                className={styles.eventContainer}
                style={screenMediaQuery && !isCurrentViewMonth ? { overflowY: "auto" } : undefined}>
          ...
          <DialogSurface className={styles.eventsDialog}>
            <DialogBody>
              <DialogTitle>
                <Title3>
                  Lezioni{" "}
                  {now.toLocaleDateString() === day.toLocaleDateString() ?
                    "di oggi"
                  : `del ${day.toLocaleDateString([], { day: "numeric", month: "long" })}`}
                </Title3>
              </DialogTitle>
              <DialogContent>{renderDetailedEvents(filteredCalendars, filteredEvents)}</DialogContent>
              <DialogActions>
                <DialogTrigger>
  ...
  return (
    <div
      className={
        screenMediaQuery ?
          mergeClasses(styles.container, styles.mobileMargins)
        : mergeClasses(styles.container, styles.sideMargin)
      }>
      <Card className={styles.toolbar}>
        ...
        <div className={styles.stack}>
          <Button
            icon={isCurrentViewMonth ? <CalendarWeekNumbersRegular /> : <CalendarMonthRegular />}
            onClick={() => setIsCurrentViewMonth(!isCurrentViewMonth)}>
            {isCurrentViewMonth ? "Settimana" : "Mese"}
          </Button>
      ...
      <div className={styles.drawerContainer}>
        <div className={styles.container}>
          <Card
            ...
            style={
              !isCurrentViewMonth && screenMediaQuery ?
                { gridTemplateColumns: "repeat(" + calendarView.length + ", 1fr)" }
              : undefined
            }>
            {screenMediaQuery ?
              isCurrentViewMonth ?
                calendarLocal.weekDaysAbbr.map((day) => (
                  <Subtitle1 key={day} className={styles.headerItem}>
                    {day}
                  </Subtitle1>
                ))
              : calendarView.map((day) => (
                  <Subtitle1 key={day.getTime()} className={styles.headerItem}>
                    {day.toLocaleDateString([], { weekday: "short" }).charAt(0).toUpperCase() +
                      day.toLocaleDateString([], { weekday: "short" }).slice(1)}
                  </Subtitle1>
                ))

            : calendarLocal.weekDays.map((day) => (
                <Subtitle1 key={day} className={styles.headerItem}>
                  {day}
                </Subtitle1>
              ))
        ...
        <Drawer
          type={window.matchMedia("(max-width: 1000px)").matches ? "overlay" : "inline"}
          open={isDrawerOpen}
          ...
          <DrawerBody className={styles.drawerBody}>
            ...
            <Button appearance="primary" onClick={onAddCalendarClick}>
              Aggiungi
            </Button>
