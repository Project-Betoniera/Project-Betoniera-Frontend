...
export function Classroom() {
  ...
  useEffect(() => {
    ...
    if (!showSideSpinner) setClassrooms(null);
  ...
  const renderClassrooms = () => {
    if (!classrooms) {
      ...
      return filteredClassrooms.map((item) => {
        ...
        if (!item.status.statusChangeAt) changeTime = "Nessun evento programmato.";
        ...
        else changeTime = item.status.statusChangeAt.toLocaleString([], { dateStyle: "medium", timeStyle: "short" });
        ...
        return (
          <Card
            ...
            className={mergeClasses(
              globalStyles.card,
              item.status.isFree ? themeStyles.cardFree : themeStyles.cardBusy,
            )}
            onClick={() => {
              setEventDialog({
              ...
              requests.event.byClassroom(start, end, item.classroom.id).then((events) =>
            ...
            <div>
              ...
              <br />
              <Body1>
                {getClockEmoji(item.status.statusChangeAt)} {changeTime}
              </Body1>
            </div>
          </Card>
        );
  ...
  function onFilterChange(_event: ChangeEvent<HTMLSelectElement>, data: SelectOnChangeData) {
    if (data.value !== "all" && data.value !== "free" && data.value !== "busy") return;
        ...
        <Dialog
          ...
          <DialogSurface>
            <DialogBody>
              ...
              <DialogContent className={globalStyles.list}>{renderEvents()}</DialogContent>
              <DialogActions>
                <DialogTrigger>
