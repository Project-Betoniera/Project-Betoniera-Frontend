...
export function PwaContextProvider({ children }: { children: JSX.Element }) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      (navigator as any).getInstalledRelatedApps().then((result: { id?: string; platform: string; url?: string }[]) => {
        if (result.length > 0) setInstalled(true);
      });
    } catch (error) {
      console.error(error);
    }
  ...
  useEffect(() => {
    ...
    return () => {
      window.removeEventListener("beforeinstallprompt", catchEvent);
    };
  }, []);
