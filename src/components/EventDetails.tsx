...
const EventDetails: FunctionComponent<EventDetailsProps> = (props: EventDetailsProps) => {
  ...
  const content = (
    <div className={styles.root}>
      ...
      {now && props.event.start <= now && props.event.end > now && (
        <Body2 className={globalStyles.blink}>{"\u{1F534}"} In corso</Body2>
      )}
  ...
  return props.as === "card" ?
      <Card
        ...
        style={
          !isOngoning ?
            { backgroundColor: props.backgroundColor ? props.backgroundColor : tokens.colorBrandBackground2Hover }
          : undefined
        }
