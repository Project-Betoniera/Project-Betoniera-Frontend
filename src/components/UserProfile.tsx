  ...
  return (
    <div>
      <div className={styles.userContainer}>
        <Avatar
          size={48}
          badge={user.isAdmin ? { icon: <CheckmarkStarburstFilled color="gold" />, size: "medium" } : undefined}
        />
      ...
      <div className={styles.profileMenu}>
        ...
        <Dropdown
          onOptionSelect={(_event, data) => setTheme(data.selectedOptions[0] as AppTheme)}
          value={themeValues[theme]}
          selectedOptions={[theme]}>
          <Option value="auto" text="Automatico">
            Automatico
          </Option>
          <Option value="light" text="Chiaro">
            Chiaro
          </Option>
          <Option value="dark" text="Scuro">
            Scuro
          </Option>
        </Dropdown>
        <Subtitle2>Utilità</Subtitle2>
        <Button
          icon={<CommentDismissFilled />}
          onClick={() => {
            localStorage.removeItem("dismissedMessages");
            reloadMessages();
          }}>
          Reimposta messaggi ignorati
        </Button>
      </div>
    </div>
  );
